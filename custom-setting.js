...
function getActiveProjectName() {
    try {
        ...
        console.error("Error reading active project:", err);
    }
    return null;
}
...
module.exports = {
    ...
    //httpServerOptions: { },
    httpNodeCors: {
        origin: ["http://localhost:3000", "http://127.0.0.1:3000"],  // Specific dev origins - no '*'
        methods: ["GET", "POST", "PUT", "DELETE", "OPTIONS"],  // Include OPTIONS for preflight
        allowedHeaders: ["Content-Type", "Authorization", "X-User-Role"],  // Allow Bearer token and custom
        exposedHeaders: [],  // Optional
        credentials: false,  // FALSE for token auth (no cookies). Set true only if using sessions/cookies
        maxAge: 600  // Cache preflight
    },

    // If needed for editor API calls
    httpAdminCors: {
    ...
    openapi: {
        template: {
            ...
            paths: {
                "/project/*": {
                    get: {
                        summary: "Serve ReactJS SPA",
                        description: "Serves the uibuilder ReactJS single-page application",
                        tags: ["SPA"],
                        parameters: [
                            {
                                name: "page",
                                ...
                                required: false,
                                schema: { type: "string" },
                                description: "Optional SPA page identifier"
                            }
                        ],
                        produces: ["text/html"],
                        responses: {
    ...
    // httpNodeSwagger: {
    //     template: path.join(__dirname, '/projects/RoomBooking/swagger_template.json')
    // },
